...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                ...
                prob_dist = util.pdf(distance, Const.SONAR_STD, observedDist)
                self.belief.setProb(row, col, prob_dist * self.belief.getProb(row, col))
    ...
    def elapseTime(self) -> None:
        ...
        for (oldTile, newTile), transProb in self.transProb.items():
            updated_belief.addProb(newTile[0], newTile[1], transProb * self.belief.getProb(oldTile[0], oldTile[1]))
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        updated_particles = collections.defaultdict(float)

        for (row, col), particle_count in self.particles.items():
            x, y = util.colToX(col), util.rowToY(row)
            distance = math.sqrt((x - agentX) ** 2 + (y - agentY) ** 2)
            prob_dist = util.pdf(distance, Const.SONAR_STD, observedDist)
            updated_particles[(row, col)] = prob_dist * particle_count

        updated_particle_counts = collections.defaultdict(int)

        for _ in range(self.NUM_PARTICLES):
            updated_particle_counts[util.weightedRandomChoice(updated_particles)] += 1

        self.particles = updated_particle_counts
    ...
    def elapseTime(self) -> None:
        ...
        for tile, particle_count in self.particles.items():
            for _ in range(particle_count):
                updated_particle_counts[util.weightedRandomChoice(self.transProbDict[tile])] += 1
