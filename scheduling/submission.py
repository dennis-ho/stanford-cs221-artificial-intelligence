...
def create_nqueens_csp(n: int = 8) -> CSP:
    ...
    for idx, variable in enumerate(variables):
        csp.add_variable(variable, [(idx, i) for i in range(0, n)])

    for i1, var1 in enumerate(variables):
        for i2, var2 in enumerate(variables):
            if i1 == i2:
                continue
            csp.add_binary_factor(var1, var2, lambda x, y: x[1] != y[1])
            csp.add_binary_factor(var1, var2, lambda x, y: abs(x[0] - y[0]) != abs(x[1] - y[1]))
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                for cid in request.cids:
                    session = (cid, quarter)
                    minUnits, maxUnits = self.bulletin.courses[cid].minUnits, self.bulletin.courses[cid].maxUnits
                    csp.add_variable(session, list(range(minUnits, maxUnits + 1)) + [0])
                    vars.append(session)
                    csp.add_binary_factor((request, quarter), session, lambda quarterlyRequest, session: session > 0 if quarterlyRequest == cid else session == 0) 
            units = create_sum_variable(csp, quarter, vars, self.profile.maxUnits)
            csp.add_unary_factor(units, lambda x: self.profile.minUnits <= x <= self.profile.maxUnits)
