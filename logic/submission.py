...
def liar() -> Tuple[List[Formula], Formula]:
    ...
    formulas.extend([
        Equiv(TellTruth(john), CrashedServer(nicole)),
        Equiv(TellTruth(nicole), CrashedServer(susan)),
        Equiv(TellTruth(susan), Not(TellTruth(nicole))),
        Exists('$x', And(TellTruth('$x'), Forall('$y', Implies(TellTruth('$y'), Equals('$x', '$y'))))),
        Exists('$x', And(CrashedServer('$x'), Forall('$y', Implies(CrashedServer('$y'), Equals('$x', '$y'))))),
    ])
...
def ints() -> Tuple[List[Formula], Formula]:
    ...
    formulas = [
        Forall('$x', Exists('$y', And(And(Successor('$x', '$y'), Not(Equals('$x', '$y'))), Forall('$z', Implies(Successor('$x', '$z'), Equals('$y', '$z')))))),
        Forall('$x', And(Or(Even('$x'), Odd('$x')), Not(And(Even('$x'), Odd('$x'))))),
